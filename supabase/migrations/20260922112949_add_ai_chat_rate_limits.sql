...
create table private.ai_chat_request_events (
...
create index ai_chat_request_events_user_time_idx
on private.ai_chat_request_events (user_id, requested_at desc);

create index ai_chat_request_events_ip_time_idx
on private.ai_chat_request_events (ip_hash, requested_at desc);

create table private.ai_chat_request_locks (
...
alter table private.ai_chat_request_events enable row level security;
alter table private.ai_chat_request_locks enable row level security;

revoke all on table private.ai_chat_request_events from public, anon, authenticated;
revoke all on table private.ai_chat_request_locks from public, anon, authenticated;
revoke all on sequence private.ai_chat_request_events_id_seq from public, anon, authenticated;
...
begin
    ...
    perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('ai-chat:ip:' || p_ip_hash, 0));
    perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('ai-chat:user:' || p_user_id::text, 0));
...
create or replace function public.finish_ai_chat_request(
    p_user_id uuid,
...
begin
    ...
    return released_user_id is not null;
end;
$$;

revoke all on function public.begin_ai_chat_request(uuid, text)
from public, anon, authenticated;
revoke all on function public.finish_ai_chat_request(uuid, uuid)
from public, anon, authenticated;

grant execute on function public.begin_ai_chat_request(uuid, text)
to service_role;
grant execute on function public.finish_ai_chat_request(uuid, uuid)
to service_role;

comment on function public.begin_ai_chat_request(uuid, text) is
'Atomically enforces AI chatbot user/IP limits and one active request per user.';
comment on function public.finish_ai_chat_request(uuid, uuid) is
'Releases an AI chatbot request lock after request processing completes.';
